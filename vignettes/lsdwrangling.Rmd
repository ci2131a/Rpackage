...
Different types of survival models require different formats for their data in order to be applied. Transitioning between formats requires in detailed knowledge of the model assumptions and how variables are treated. The `lsdwrangling` package in R seeks to provide methods for transforming data between usable formats in an effort to ease this process `lsdwrangling` can transition between the counting process format and longitudinal or panel format. 
...
data1 <- data.frame(sid = c(1,1,1,2,2,3),
...
```{r, echo = FALSE, results = "asis"}
data2 <- data.frame(sid = c(1,1,1,2,2,3),
                     start = c(0,31,64,0,33,0),
                     stop = c(31,64,96,33,59,28),
                     status = c(0,0,1,0,0,1),
